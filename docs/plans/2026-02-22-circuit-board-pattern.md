# CircuitBoardPattern Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a circuit board audio visualizer (Key 8) with Manhattan-routing traces that grow and animate with audio reactivity.

**Architecture:** Single pattern class inheriting from BasePattern. Grid-based node placement with orthogonal circuit traces (Manhattan routing). Traces grow from nodes over time with data flow animation. Audio reactivity: Bass spawns new connections, Mid accelerates data flow, Treble pulses nodes.

**Tech Stack:** p5.js (WEBGL mode), TypeScript, existing BasePattern infrastructure

---

## Task 1: Create pattern class structure and parameter interface

**Files:**
- Create: `src/patterns/CircuitBoardPattern.ts`
- Modify: `src/patterns/index.ts` (add export)

**Step 1: Write the type definitions and interface**
...
**Step 2: Add export to patterns/index.ts**
...
```bash
git add src/patterns/CircuitBoardPattern.ts src/patterns/index.ts
git commit -m "feat: add CircuitBoardPattern class structure"
```

---

## Task 2: Implement grid and node initialization

**Files:**
- Modify: `src/patterns/CircuitBoardPattern.ts`

**Step 1: Add grid and node initialization methods**
...
```bash
git add src/patterns/CircuitBoardPattern.ts
git commit -m "feat: add grid and node initialization"
```

---

## Task 3: Implement Manhattan path generation

**Files:**
- Modify: `src/patterns/CircuitBoardPattern.ts`

**Step 1: Add trace creation and path generation**
...
```bash
git add src/patterns/CircuitBoardPattern.ts
git commit -m "feat: add Manhattan path generation and trace creation"
```

---

## Task 4: Implement audio-reactive update
...
```typescript
   ...
   */
  private updateTraces(p: p5, flowMult: number): void {
   ...
   */
  private updateNodes(p: p5, pulseMult: number): void {
    for (const node of this.nodes) {
      // Pulse phase increases with time
      node.pulsePhase += 0.05 * pulseMult;
      if (node.pulsePhase > p.TWO_PI) {
        node.pulsePhase -= p.TWO_PI;
      }
    }
  }
```
...
```bash
git add src/patterns/CircuitBoardPattern.ts
git commit -m "feat: add audio-reactive update"
```

---

## Task 5: Implement rendering

**Files:**
- Modify: `src/patterns/CircuitBoardPattern.ts`

**Step 1: Add draw methods**
...
```typescript
  ...
  private drawDataFlow(p: p5, trace: CircuitTrace, hue: number): void {
    const totalLength = this.getPathLength(trace.path);
   ...
   */
  private getPathLength(path: p5.Vector[]): number {
    let length = 0;
...
```bash
git add src/patterns/CircuitBoardPattern.ts
git commit -m "feat: add circuit board rendering"
```

---

## Task 6: Integrate pattern into application
...
Run: `npm run dev`
Navigate to app, press Key 8
Expected: Circuit board pattern appears

**Step 5: Commit**

```bash
git add src/App.tsx
git commit -m "feat: add CircuitBoardPattern to app (Key 8)"
```

---

## Task 7: Update IMPLEMENTATION_HISTORY.md

**Files:**
- Modify: `IMPLEMENTATION_HISTORY.md`

**Step 1: Add CircuitBoardPattern entry**

Add to "実装されたビジュアルパターン" section:
...
```bash
git add IMPLEMENTATION_HISTORY.md
git commit -m "docs: add CircuitBoardPattern to implementation history"
```
...
- [ ] Pattern loads on Key 8
- [ ] Circuit board renders correctly with nodes and traces
- [ ] Traces grow from nodes over time
- [ ] Data flow animation works
- [ ] Audio reactivity works (Bass→spawn, Mid→flow, Treble→pulse)
- [ ] Node types have different shapes
- [ ] Trace layers have different colors
- [ ] MIDI Learn can assign any CC to any parameter
- [ ] Build passes
- [ ] No console errors
...
1. Start app, switch to Key 8
2. Verify circuit board appears with nodes and traces
3. Verify traces grow from nodes over time
4. Play music with strong bass → verify new traces spawn
5. Adjust gridSpacing parameter → verify node positions change
6. Adjust baseHue parameter → verify colors change
7. Adjust growthSpeed → verify traces grow faster/slower
8. Test MIDI Learn on various parameters
9. Verify pattern switching works (to/from Key 8)
